package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1517:uint = 1;
      
      protected static const const_1516:uint = 2;
      
      protected static const const_1514:uint = 4;
      
      protected static const const_1515:uint = 3;
      
      protected static const name_2:uint = 7;
      
      public static const const_1253:int = 5;
      ...
      protected var var_254:URLRequest;
      
      protected var var_733:XML;
      
      protected var var_613:Class;
      ...
      protected var var_2583:Boolean = true;
      ...
      {
         return !!this.var_254 ? this.var_254.url : null;
      }
      ...
      {
         return !!this.var_926 ? uint(this.var_1161 - this.var_1865) : uint(getTimer() - this.var_1865);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_347,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_39.close();
            }
      ...
      public function load(param1:URLRequest, param2:int = 5) : void
      {
         ...
         this._name = parseNameFromUrl(this.var_254.url);
      ...
      public function resume() : void
      {
         ...
         {
            this.var_927 = false;
            if(!this.var_926 && this.var_254)
            {
               this.var_1161 = -1;
               this.var_1865 = getTimer();
               this.var_39.load(this.var_254,this.var_16);
            }
         }
      }
      ...
      {
         if(!this.var_926 && !_disposed && this.var_1160 > 0)
         {
            try
            {
               this.var_39.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_254,const_1253 - this.var_1160);
            if(this.var_2583)
            {
               this.var_254.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_254.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_2583 = false;
            }
            --this.var_1160;
            this.var_411 = 0;
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               ...
               this.debug("Load event STATUS " + this.var_347 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_347,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_347,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_347))
               {
            ...
            case TimerEvent.TIMER:
               ...
               if(!_disposed)
               {
                  this.var_411 = LibraryLoader.name_2;
         ...
         if(this.var_411 == LibraryLoader.const_1515)
         {
         ...
         if(this.var_411 == LibraryLoader.name_2)
         {
            if(this.prepareLibrary())
            {
               ...
               this.var_1161 = getTimer();
               this.removeEventListeners();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_347,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_613 = this.getDefinition(this._name) as Class;
         if(this.var_613 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_254.url + "!");
            }
            return false;
         }
         try
         {
            xmlClass = this.var_613.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
         ...
         try
         {
            this.var_733 = new XML(bytes.readUTFBytes(bytes.length));
         }
         catch(e:Error)
         {
            if(!retry())
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_347,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_347,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
