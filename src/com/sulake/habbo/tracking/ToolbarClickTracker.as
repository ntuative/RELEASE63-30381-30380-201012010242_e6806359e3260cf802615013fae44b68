package com.sulake.habbo.tracking
{
   import com.sulake.habbo.configuration.IHabboConfigurationManager;
   
   public class ToolbarClickTracker
   {
       
      
      private var var_1170:IHabboTracking;
      
      private var var_1411:Boolean = false;
      
      private var var_2542:int = 0;
      
      private var var_1793:int = 0;
      
      public function ToolbarClickTracker(param1:IHabboTracking)
      {
         super();
         this.var_1170 = param1;
      }
      
      public function dispose() : void
      {
         this.var_1170 = null;
      }
      
      public function configure(param1:IHabboConfigurationManager) : void
      {
         this.var_1411 = Boolean(parseInt(param1.getKey("toolbar.tracking.enabled","1")));
         this.var_2542 = parseInt(param1.getKey("toolbar.tracking.max.events","100"));
      }
      
      public function track(param1:String) : void
      {
         if(!this.var_1411)
         {
            return;
         }
         ++this.var_1793;
         if(this.var_1793 <= this.var_2542)
         {
            this.var_1170.track("toolbar",param1);
         }
      }
   }
}
