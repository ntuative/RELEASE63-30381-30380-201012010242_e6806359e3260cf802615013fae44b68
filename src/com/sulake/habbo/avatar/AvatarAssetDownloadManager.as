package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private const const_1818:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_817 = new Timer(this.const_1818,1);
         this.var_817.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_376)
         {
            this.var_376.addEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_376.addEventListener(AssetLoaderEvent.const_44,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_376)
         {
            this.var_376.removeEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_376.removeEventListener(AssetLoaderEvent.const_44,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_818);
            if(this.var_1241.indexOf("?") > 0)
            {
               _loc2_ = this.var_1241 + "&retry=" + this.var_818;
            }
            else
            {
               _loc2_ = this.var_1241 + "?retry=" + this.var_818;
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_1561 = true;
         for each(_loc2_ in this.var_1003)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2199,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1004[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1004[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:AvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1561)
         {
            this.var_1003.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_462[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_462[_loc3_] = _loc6_;
            }
            this.var_819[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_819)
         {
            _loc7_ = true;
            _loc4_ = this.var_819[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_462[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_819[_loc3_];
         }
         while(_loc6_ < this.var_669.length)
         {
            _loc5_ = this.var_669[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_669.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_817.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_670.length > 0 && this.var_669.length < this.const_1506)
         {
            _loc1_ = this.var_670[0];
            _loc1_.startDownloading();
            this.var_669.push(this.var_670.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_670.indexOf(param1) == -1 && this.var_669.indexOf(param1) == -1)
         {
